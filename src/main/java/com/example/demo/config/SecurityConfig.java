...
public class SecurityConfig {
		...
		// 메소드에 @Bean을 붙이면, 반환값인 인코더 객체가 스프링 컨테이너에 등록됨 => test폴더에 etc-PasswordTest.java에 autowired로 객체 생성가능.
		// 사용자 패스워드를 암호화하는데 사용한다
		@Bean
		...
		//10장 p.24
		// 사용자 인증을 처리하는 메소드
		// 메소드에 @Bean을 붙이면, 반환값인 인증 필터 객체가 스프링 컨테이너에 등록됨	 => MemberServiceImpl.java에 객체생성해서 사용.	
		@Bean
